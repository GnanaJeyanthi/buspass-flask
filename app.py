...
routes = Route.query.all()
return render_template('admin_dashboard.html', applications=applications, routes=routes)
...
route = Route(route_no=route_no, start_point=start_point, end_point=end_point,
capacity=capacity)
...
msg = Message(f'Bus Pass {status}', sender=app.config['MAIL_USERNAME'],
recipients=[student.email])
msg.html = render_template('email_template.html', name=student.name,
route_no=route.route_no, start_point=route.start_point, end_point=route.end_point,
status=status)
mail.send(msg)
...
create_tables()
print("Tables created and default admin added (email: admin@gmail.com, password:
admin123)")
